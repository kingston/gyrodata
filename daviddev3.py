...
def extractFeatures(features, entry, config):
    ...
    if config['data-filters']['accfile']:
        ...
        #pylab.show()
        acc=bodyAccX+bodAccY+bodyAccZ
        vel = 0
        for i in range(1,len(acc)):
            vel = vel + (acc[i]+acc[i-1])/2.0
        features.append(float(vel)/len(acc))
        
        features.append(max(acc))
        features.append(min(acc))
        features.append(max(acc)-min(acc))
    ...
    if config['data-filters']['gyrofile']:
        ...
        peaks = buttergyroY[peakind]
        features.append(median(peaks))
        
        #features.append(min(buttergyroY))
        #features.append(max(buttergyroY)-min(buttergyroY))
